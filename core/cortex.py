...
class Cortex:
    ...
    def _system_prompt(self, sense: dict) -> str:
        ...
        return (
            ...
            "- The <NOW> block's \"focus\" is the ONLY truth about focus sessions: if "
            "focus.active is false, NEVER mention a focus session or minutes left, "
            "even if older messages talked about one — never echo focus phrases "
            "from history.\n"
            "- If asked which model you are, answer with the EXACT provider + model "
            "from <NOW>'s \"llm\" field (e.g. DeepSeek · deepseek-chat). Never hedge "
            "with 'frontier-class'.\n"
